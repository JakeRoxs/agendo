...
export class LinkService {
  ...
  private scanFolders(): string[] {
    return ["", this.config.completeFolder, this.config.cancelledFolder, this.config.backlogFolder];
  }
  ...
  async findReferencesToName(
    ...
    for (const folder of this.scanFolders()) {
      ...
      references.push(...(await this.scanFolderForName(dir, oldFileName, excluded)));
    }
   ...
   */
  async warnOnBrokenReferences(oldFileName: string, newUri: vscode.Uri): Promise<void> {
