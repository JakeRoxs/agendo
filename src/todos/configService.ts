...
/** Shape of the `.todos-config.json` file the skill reads. */
...
export class ConfigService {
    ...
    get root(): string {
        return (get<string>(Settings.Root) || "docs/todos").replace(/\\/g, "/").replace(/\/+$/, "");
    }
    ...
    /**
     * Write `.todos-config.json` into the root folder so the skill can read the
    ...
    async writeConfigFile(): Promise<void> {
        ...
        try {
            ...
            const target = vscode.Uri.joinPath(rootUri, ".todos-config.json");
            const content = JSON.stringify(this.toTodosConfig(), null, 2) + "\n";
            await vscode.workspace.fs.writeFile(target, Buffer.from(content, "utf8"));
            out`Wrote ${target.fsPath}`;
        } catch (error) {
            out`Failed to write .todos-config.json: ${error}`;
        }
    }
